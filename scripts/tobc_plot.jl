...
# ╔═╡ b72eea6a-8728-4edd-accf-dcd8fa6e930e
using JLD2

# ╔═╡ 45cb0c6d-847f-4a8c-9bd4-b19fbabb7c3d
using CairoMakie

# ╔═╡ ad2fdcd7-7c3d-4050-bd61-20f4f403c090
using LaTeXStrings

# ╔═╡ b847bf6c-9847-48b7-911b-928a632aa42e
using LinearAlgebra

# ╔═╡ f7c9f0e7-77b7-4b43-9581-57d39e5e1b5e
name = "tobc_3_2.7"

# ╔═╡ c857141c-7684-4fea-9687-a984579498c6
file = jldopen(joinpath(@__DIR__, "..", "data", "exact", "$name.jld2"))

# ╔═╡ af42c749-5aa2-45cd-8642-29eb486c848a
shape = (2, :)

# ╔═╡ b3498cb2-0965-4d2c-a830-e2f8a4a5390d
data_keys = reshape([k for k in keys(file) if !(k in ("n_sites", "T", "t_range"))], shape)

# ╔═╡ 501eae63-f023-45ef-8041-7223994d17d2
ts = file["t_range"]

# ╔═╡ 81b40b69-1395-4d8a-ba30-7d588c504096
L = length(ts)

# ╔═╡ 252588e0-e7fe-4f01-8b08-d321289dd605
data = let
	data = map(data_keys) do k
		raw = abs.(real(file[k][1:L, 1:L]))
		Symmetric(Matrix(map(x->ismissing(x) ? 0.0 : x, raw)))
	end
	reshape(data, shape)
end

# ╔═╡ cb2c2888-0e40-4613-9297-1b8fe19e1f7e
data[4]

# ╔═╡ 81cd7e53-167e-4e22-b9a3-0d7b2c11c103
min_d = minimum(map(minimum, data))

# ╔═╡ 12897bae-e20b-4c84-bda3-530c3c25f9cf
max_d = maximum(map(maximum, data))

# ╔═╡ 7287819e-d0b0-46b0-8913-0e746fab0d76
figsize = (400 * size(data, 2), 400 * size(data, 1))

# ╔═╡ 782dd73b-9f71-4969-b0ca-c9968895e740
let fig = Figure(size=figsize, fontsize=22, interpolate=true)
	for (x, row) in enumerate(eachrow(data)), (y, d) in enumerate(row)
...
manifest_format = "2.0"
project_hash = "f9adee763f319a34c12ee3726ef6f8a0d8660b89"
...
git-tree-sha1 = "60c334d923094a5fa4c2f65899b212261d429323"
